...
export const Navbar = ( {isDarkMode, handleToggleDarkMode} )=>{
    ...
    return(
        <nav className="navbar">
            ...
            {!isMobile &&  <DesktopNav handleToggleDarkMode={handleToggleDarkMode} changeDarkModeBtn={changeDarkModeBtn}/>}
            {isMobile && (
                <>
                {renderMobile()}
                <span className="toggleMobileNav" onClick={ ()=> setIsOpen(prevIsOpen => !prevIsOpen)}>
                    {
                        isOpen ? 
                        <img src={closeMenu} style={{width:"20px"}} alt="open menu"/> : 
                        <img src={hamburgerBtn} style={{width:"20px"}} alt="open menu"/>
                    }
                </span>
                </>
            )}
